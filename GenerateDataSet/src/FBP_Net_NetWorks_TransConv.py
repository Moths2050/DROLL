...
class FBP_Net(nn.Module):
    def __init__(self, args, out_ch_pre=96, out_ch_post=96):
        ...
        self.Sino_Spa2Fre = Spatial2FrequencyLayer.Spatial2FrequencyLayer()

        self.Sino_In = unet_parts.DoubleConv(2, 64, mid_channels=None, kernel_sizes=[1, 3], strides=[1, 1], paddings=[0, 1])
        ...
        self.Sino_Out = unet_parts.OutConv_Proj(64, 2, kernel_sizes=[1, 1], strides=[1, 1], paddings=[0, 0])

        self.Sino_Fre2Spa = Frequency2SpatialLayer.Frequency2SpatialLayer()
        ...
        self.Img_Down4 = unet_parts.Down(512, 1024)
        ...
        self.Img_Up1 = unet_parts.Up(1024, 512, bilinear=False)
        self.Img_Up2 = unet_parts.Up(512, 256, bilinear=False)
        self.Img_Up3 = unet_parts.Up(256, 128, bilinear=False)
    ...
    def forward(self, x):
        # Sinogram Domain
        # Spa2Fre
        SinoSpa2Fre = self.Sino_Spa2Fre(x)
        # print(x.shape)
        Sino1 = self.Sino_In(SinoSpa2Fre)
        # print(Sino1.shape)
        ...
        # print(SinoOut.shape)
        # Fre2Spa
        SinoFre2Spa = self.Sino_Fre2Spa(SinoOut)

        # BP Layer
        Img = self.FBP(SinoFre2Spa)
        # print(Img.shape)
