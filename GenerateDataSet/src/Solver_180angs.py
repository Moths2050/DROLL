...
from src import FBP_Net_Networks_Interpolate
...
class Solver(object):
    def __init__(self, args, data_loader):
        ...
        self.save_test_path = args.save_iters_result

        self.FBP_Net = FBP_Net_Networks_Interpolate.FBP_Net(args)
        # print(self.FBP_Net)
        ...
        self.criterionSSIMLoss = SSIM.Structural_loss()
    ...
    def train(self):
        ...
        for epoch in range(0, self.num_epochs):
            ...
            for iter_, (x, y) in enumerate(self.data_loader):
                ...
                # 复合loss
                mseLoss = self.criterionMSELoss(pred, y)
                ssimLoss = self.criterionSSIMLoss(pred, y)
                # print(mseLoss)
                # print(ssimLoss*500)
                # fit hyper primeter
    ...
    def test(self):
        del self.FBP_Net
        # load
        self.FBP_Net = FBP_Net_Networks_Interpolate.FBP_Net(self.args ).to(self.device)
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(self.data_loader):
                ...
                img.save(os.path.join(self.args.save_path, self.save_test_path, f_name_res))

                img = Image.fromarray(np.uint16(res_y * 65535))
                img.save(os.path.join(self.args.save_path, self.save_test_path, f_name_y))
        print("#############################")
